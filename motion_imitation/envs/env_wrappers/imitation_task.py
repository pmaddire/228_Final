...
class ImitationTask(object):
  ...
  def __init__(self,
    ...
    self.physics_weight_scale = physics_weight_scale

  # physics params
    self._physics_weights = {
        'dynamics': 0.1,
        'power': 0.05,
        'zmp': 0.1,
        'ang_momentum': 0.05
  ...
  def reward(self, env):
    ...
    physics = self._calc_physics_constraints()
    reward = (self._pose_weight * pose_reward 
             ...
             + self.physics_weight_scale *np.exp(-0.1 * physics['dynamics_err'])      # Dynamics compliance
             + self.physics_weight_scale *np.exp(-0.05 * physics['power'])          # Energy efficiency
             + self.physics_weight_scale *(1 - physics['zmp_err'])                   # Stability
             + self.physics_weight_scale *np.exp(-0.05 * physics['ang_momentum_err'])  # Angular momentum
    )
...
# my added reward: 
  ...
  def _calc_physics_constraints(self):
      """Calculate physics-based constraint violations."""
      robot = self._env.robot
      pyb = self._get_pybullet_client()
      
      # Initialize history if not exists
      if not hasattr(self, '_com_vel_history'):
          self._com_vel_history = collections.deque(maxlen=5)
      
      # 1. Newton-Euler Dynamics Check
      current_vel = np.array(robot.GetBaseVelocity()[0])
      self._com_vel_history.append(current_vel)
      
      dyn_err = 0
      if len(self._com_vel_history) >= 2:
          # Use central difference for acceleration
          acc = (self._com_vel_history[-1] - self._com_vel_history[-2]) / self._env.env_time_step
          expected_acc = np.array([0, 0, -9.8])  # Gravity
          dyn_err = np.sum((acc - expected_acc)**2)

      # 2. Energy Conservation
      joint_torques = robot.GetMotorTorques()
      joint_vels = robot.GetMotorVelocities()
      power = np.sum(np.abs(np.array(joint_torques) * np.array(joint_vels)))
      
      # 3. ZMP Stability (simplified)
      foot_contacts = robot.GetFootContacts()
      zmp_err = 0 if any(foot_contacts) else 1  # Simplified - just check if any foot is in contact
      
      # 4. Angular Momentum
      ang_momentum = np.array(robot.GetBaseAngularVelocity())
      ang_momentum_err = np.sum(ang_momentum**2)
      
      return {
          'dynamics_err': dyn_err,
          'power': power,
          'zmp_err': zmp_err,
          'ang_momentum_err': ang_momentum_err
      }

  def _estimate_com_acceleration(self, window=5):
      """Estimate COM acceleration using finite differences."""
      if not hasattr(self, '_com_vel_history'):
          self._com_vel_history = collections.deque(maxlen=window)
  ...
  def _calculate_zmp(self, foot_forces):
      """Calculate Zero Moment Point from foot forces."""
      foot_positions = [robot.GetFootPosition(i) for i in range(4)]
      total_force = np.sum(foot_forces)
      
      if total_force < 1e-6:  # No contact
          return np.zeros(2)
      
      zmp = np.zeros(2)
      for i in range(4):
          zmp += foot_positions[i][:2] * foot_forces[i]
      return zmp / total_force

  def _is_in_support_polygon(self, zmp, support_polygon):
      """Check if ZMP is within support polygon."""
      # Implement point-in-polygon test
      # (Can use ray casting or winding number algorithm)

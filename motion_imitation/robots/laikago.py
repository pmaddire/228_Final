...
class Laikago(minitaur.Minitaur):
  ...
  def GetFootContacts(self):
      """Returns a list of booleans indicating foot contact (one per foot)."""
      all_contacts = self._pybullet_client.getContactPoints(bodyA=self.quadruped)

      # Ensure the list matches the number of feet
      contacts = [False] * len(self._foot_link_ids)
      ...
      for contact in all_contacts:
          ...
          try:
              ...
              if 0 <= toe_link_index < len(contacts):
                  contacts[toe_link_index] = True
          except ValueError:
              continue  # Contact not with a foot

      return contacts
  
  def GetBaseAngularVelocity(self):
      """Returns the angular velocity of the robot base (in world frame)."""
      _, ang_vel = self._pybullet_client.getBaseVelocity(self.quadruped)
      return ang_vel
